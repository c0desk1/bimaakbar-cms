...
const parseMetadata = (items: any[]) => items ? items.map((item: any) => ({ ...item, metadata: JSON.parse(item.metadata as string || '{}') })) : [];
...
app.get('/api/posts', async (c) => {
  try {
    if (!c.env.DB) {
        return c.json({ error: 'Database binding not found', message: 'D1 database binding is missing in the environment' }, 500);
    }
...
app.get('/api/posts/:slug', async (c) => {
  try {
    ...
    console.error("Error in /api/posts/:slug:", e);
    return c.json({ error: 'Gagal mengambil postingan', message: e.message }, 500);
  }
});
...
app.get('/api/portofolio', async (c) => {
  try {
    ...
    console.error("Error in /api/portofolio:", e);
...
app.get('/api/pages', async (c) => {
  try {
    ...
    console.error("Error in /api/pages:", e);
...
app.get('/api/pages/:slug', async (c) => {
  try {
    ...
    console.error("Error in /api/pages/:slug:", e);
    return c.json({ error: 'Gagal mengambil halaman', message: e.message }, 500);
  }
});
...
// Get comments for a post and handle view count
app.get('/api/comments', async (c) => {
  try {
    ...
    console.error("Error in /api/comments:", e);
